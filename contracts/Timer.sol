...
contract Timer is IntAccessI {
        ...
        isIntAuth
    {
        ...
        // if the target address is not defined set it to the msg.sender's address
        if (_targetAdr == address(0x0))
            _targetAdr = msg.sender;

        // Add the new notification to the array in mapping
        notification[_dateTime / 10].push(NotificationEntry({
            notificationAddress: _targetAdr,
            subject: _subject,
            message: _message
        }));
                ...
                for (uint j_10_S = i_100_S * 10; j_10_S < (i_100_S + 1) * 10 ; j_10_S++) {
                    ...
                    if ((j_10_S >= nextPingExec_10_S) && (j_10_S <= timeNow_10_S)) {
                        ...
                        for (uint k=0; k<notification[j_10_S].length; k++) {
                            ...
                            uint rescheduleTimeDelta = 
                                NotificationI(notification[j_10_S][k].notificationAddress).ping(notification[j_10_S][k].subject, notification[j_10_S][k].message, j_10_S * 10);
