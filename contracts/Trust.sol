...
contract Trust is IntAccessI, ExtAccessI {
    ...
    function Trust() IntAccessI(msg.sender) ExtAccessI(msg.sender) public {
        // Add log entry
        LogTrust(bytes32("TrustContractCreation"), address(this), 0x0, now);
    }
     ...
     */
    function initEcosystem(address _poolAdr, address _bondAdr, address _bankAdr, address _policyAdr,
        address _settlementAdr, address _adjustorAdr, address _timerAdr, bool _isWinterTime)
        public
        isExtAuth
    {
        ...
        // Add log entries
        LogTrust(bytes32("SetTrustAddress"), address(this), 0x0, now);
        LogTrust(bytes32("SetPoolAddress"), _poolAdr, 0x0, now);
        LogTrust(bytes32("SetBondAddress"), _bondAdr, 0x0, now);
        LogTrust(bytes32("SetBankAddress"), _bankAdr, 0x0, now);
        LogTrust(bytes32("SetPolicyAddress"), _policyAdr, 0x0, now);
        LogTrust(bytes32("SetSettlementAddress"), _settlementAdr, 0x0, now);
        LogTrust(bytes32("SetAdjustorAddress"), _adjustorAdr, 0x0, now);
        LogTrust(bytes32("SetTimerAddress"), _timerAdr, 0x0, now);
        ...
        isExtAuth
    {
        ...
        LogTrust(bytes32("ChangeInDaylightSaving"), msg.sender, 0x0, now);
	}
